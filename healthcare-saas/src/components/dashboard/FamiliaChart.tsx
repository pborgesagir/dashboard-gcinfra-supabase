...
export default function FamiliaChart({ data, loading, chartType = 'bar' }: FamiliaChartProps) {
        ...
        <CardContent>
          ...
          <Box display="flex" justifyContent="center" alignItems="center" height={400}>
            <Typography color="text.secondary">
              Nenhum dado de família disponível para exibir o gráfico
            </Typography>
          </Box>
        </CardContent>
      </Card>
  ...
  const formatPieTooltip = (value: number, name: string) => {
    const item = sortedData.find(d => d.familia === name)
    return [
      ...
      >
        ...
        >
          {top10Data.map((entry, index) => (
            <Cell 
              key={`cell-${index}`} 
              fill={index === activeIndex ? '#388e3c' : '#4caf50'}
            />
          ))}
        </Bar>
      </BarChart>
    ...
    <ResponsiveContainer width="100%" height={400}>
      <PieChart>
        <Pie
          ...
          cy="50%"
          labelLine={false}
          label={({ familia, percentage }) => `${familia || 'N/A'} (${percentage.toFixed(1)}%)`}
          outerRadius={120}
          fill="#8884d8"
          dataKey="count"
          onMouseEnter={(_, index) => setActiveIndex(index)}
  ...
  const topFamilia = sortedData[0]
  const familiasWithData = sortedData.filter(item => item.familia && item.familia.trim() !== '')
    ...
    <Card>
      <CardContent>
        <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={2}>
          <Box>
            <Typography variant="h6" gutterBottom>
              {chartType === 'bar' ? 'Top 10 Ordens de Serviço por Família' : 'Ordens de Serviço por Família'}
            </Typography>
            <Typography variant="body2" color="text.secondary">
              {chartType === 'bar' ? 'As 10 principais famílias mais frequentes' : 'Distribuição das OS por família de equipamento'}
            </Typography>
          </Box>
        ...
        <Box sx={{ mt: 2, display: 'flex', gap: 3, flexWrap: 'wrap' }}>
          <Box>
            ...
            <Typography variant="h6" color="#4caf50">
              {topFamilia?.familia || 'Não informado'} ({topFamilia?.count.toLocaleString('pt-BR')} OS)
            </Typography>
          </Box>
          ...
          <Box>
            ...
            <Typography variant="h6" color="#2196f3">
              {familiasWithData.length} de {sortedData.length}
            </Typography>
          </Box>
          ...
          <Box>
            ...
            <Typography variant="h6" color="#ff9800">
              {totalOrders > 0 
                ? `${((familiasWithData.reduce((sum, item) => sum + item.count, 0) / totalOrders) * 100).toFixed(1)}%`
                : '0%'
              }
            </Typography>
          </Box>
            ...
            {sortedData.slice(0, 5).map((item, index) => (
              <Box key={index} display="flex" justifyContent="space-between" alignItems="center" mb={0.5}>
                <Typography variant="body2" sx={{ fontWeight: index === 0 ? 'bold' : 'normal' }}>
                  {index + 1}. {item.familia || 'Não informado'}
                </Typography>
                <Typography variant="body2" color="text.secondary">
